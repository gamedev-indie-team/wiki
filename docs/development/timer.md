...
!> Если не добавить `DeleteOnExpireTimer` (**None** тоже игнорируется), то таймер никогда не удалиться. После истечения участвовать в обработке не будет (не попадет в фильтр из-за `TimerExpiredTag`)
